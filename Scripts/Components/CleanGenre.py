...
def simple_clean(genre):
    ...
    S_unwanted_words = ['with', 'influences', 'earlier', 'later', 'early', 'and', 'or', 'mid']
...
def simple_clean2(genre):
    """Cleans genre while preserving word order and removing unwanted words, returning a comma-separated list."""
    # Define unwanted words to remove
    S_unwanted_words = ['with', 'influences', 'earlier', 'later', 'early', 'and', 'or', 'mid', 'metal']
    ...
    # Join all cleaned parts into a final string, separated by commas
    return ', '.join(cleaned_parts)
...
if __name__ == "__main__":
    recs = process_genres(data[data['Band ID'] == 10175], 'Genre')
    print(recs)