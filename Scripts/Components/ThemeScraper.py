...
processed = pd.read_csv(LYRICS) if os.path.exists(LYRICS) else pd.DataFrame()
...
def refresh(): 
    ...
    process_band_ids(band_ids_to_process, 200, LYRICS, scrape_band_data, delay_between_requests=0.05)
