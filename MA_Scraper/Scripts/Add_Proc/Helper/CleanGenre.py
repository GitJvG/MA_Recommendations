...
def basic_processing(df_series):
    ...
    df_series.loc[:] = df_series.str.replace(r'\(.*?\)|\s?\'n\'\s?roll', '', regex=True) # removes 'n roll and parenthesis

    main_genres_check = ['metal', 'punk', 'rock', 'metal']
    for genre in main_genres_check:
        df_series.loc[:].str.replace(rf'(?<=\b{genre})\s*/', ', ', regex=True) # replaces / with comma if preceeded by main genre
